...
* **File Storage:** The generated artifact is saved for historical purposes to:
  `workflow-agent-files/rules-generator/[timestamp]-[sanitized-name]-rules.md`
...
4. **Generation Phase:**
   ...
   * This internally uses the configured Gemini model (`google/gemini-2.0-flash-001`) along with the rules-specific system prompt (`RULES_SYSTEM_PROMPT` defined in `index.ts`) to generate the rules based on the assembled prompt.
5. **Output Processing & Saving:**
   * Formats the generated rules with a title header and timestamp.
...
flowchart TD
    ...
    G --> H[6. Save Rules to workflow-agent-files];
